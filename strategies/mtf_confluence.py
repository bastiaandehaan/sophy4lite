...
class MTFParams:
    ...
    # Optional: use EV in entry filter (default False to avoid bias)
    use_ev_in_filter: bool = False
    min_ev_R: float = 0.0  # only if use_ev_in_filter=True
...
class MTFSignals:
    ...
    def create_timeframes(self, df_m1: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        tfs["M5"] = (
            df_m1.resample("5min", label="right", closed="right").agg(agg).dropna()
        )
        tfs["M15"] = (
            df_m1.resample("15min", label="right", closed="right").agg(agg).dropna()
        )
        tfs["H1"] = (
            df_m1.resample("60min", label="right", closed="right").agg(agg).dropna()
        )
        return tfs
    ...
    # ---------- EV (ongewijzigde wiskunde; niet als hard filter gebruikt) ----------
    ...
    def calculate_expected_value(
        ...
        sl: float,
        tp: float,
        win_rate: float = 0.58,  # jouw default blijft behouden
    ) -> float:
        risk = abs(entry - sl)
        reward = abs(tp - entry)
        if risk == 0:
    ...
    def generate_signals(
        ...
        # ffill higher TFs (no look-ahead if resample closed/right)
        ...
        # EV (computed, not required for filtering unless flag true)
